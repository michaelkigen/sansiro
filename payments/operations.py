...
SAFARICOM_API = settings.SAFARICOM_API
    ...
    if json_response.get('ResponseCode'):
        if json_response["ResponseCode"] == "0":
            ...
            if transaction_id:
                ...
                print('this is the transaction id', transaction.trans_id)
                return transaction.trans_id
# ...
...
def check_payment_status(checkout_request_id, shortcode=None):
    ...
    if 'ResponseCode' in json_response and json_response["ResponseCode"] == "0":
        ...
        if transaction:
            ...
            transaction.message = response_message
            transaction.save()
            print('TRANSACION is :', transaction)
            print('TRANSACION_FINISHED is :', transaction.is_finished)
            print('TRANSACION_SUCCESSFUL is :', transaction.is_successful)
